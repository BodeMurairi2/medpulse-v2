...
class LabTestResult(Base):
    ...
    files = relationship("LabTestFile", back_populates="lab_test_result", cascade="all, delete-orphan")

    def __repr__(self):
        return f"<LabTestResult(id={self.test_id}, test_name={self.test_name})>"

# New Lab Test Files table
class LabTestFile(Base):
    __tablename__ = "lab_test_files"
    file_id = Column(Integer, primary_key=True, autoincrement=True)
    lab_test_id = Column(Integer, ForeignKey("lab_test_results.test_id"))
    file_url = Column(String, nullable=False)
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    lab_test_result = relationship("LabTestResult", back_populates="files")

    def __repr__(self):
        return f"<LabTestFile(id={self.file_id}, url={self.file_url})>"
